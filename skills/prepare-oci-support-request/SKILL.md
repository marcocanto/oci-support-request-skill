...
1. Confirm the affected service, closest symptom, region, UTC incident window, and known resource OCIDs.
2. Run `oci --version` and confirm authentication with a narrow read-only command.
3. Read [references/evidence-map.md](references/evidence-map.md) for the service inputs and issue overlay.
4. Preview the collector plan:
...
5. Show the planned commands and missing inputs. Run with `--execute` only after confirming the target tenancy, region, and resource identifiers.
6. Review every generated file before using it. The collector removes common secret-bearing fields, tags, and instance metadata, but the operator remains responsible for the final review.
7. Generate a concise SR description with problem, impact, resources, UTC timing, exact error and request IDs, expected versus observed behavior, recent changes, troubleshooting, and collected evidence.
8. If the user asks to submit or update the SR, read [references/support-cli.md](references/support-cli.md) completely before proceeding.
...
- Do not infer a tenancy, compartment, region, resource OCID, CSI, user group, severity, or 24x7 contact.
